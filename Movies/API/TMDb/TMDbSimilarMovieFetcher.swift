...
class TMDbSimilarMovieFetcher: SimilarMovieFetcher {
  ...
  func fetch(for id: Int, page: Int, completion: @escaping (SimilarMovieResponse?, APIError?) -> Void) {
    ...
    provider.request(.movieSimilars(id: id, page: page)) { result in
      switch result {
      case let .success(response):
        do {
          ...
          completion(
            SimilarMovieResponse(results: convertedListItems,
                                 page: results.page,
