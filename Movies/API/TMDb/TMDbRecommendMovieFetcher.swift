//
//  TMDbRecommendMovieFetcher.swift
//  Movies
//
...
class TMDbRecommendMovieFetcher: RecommendMovieFetcher {
  ...
  func fetch(for id: Int, page: Int, completion: @escaping (RecommendedMovieResponse?, APIError?) -> Void) {
    ...
    provider.request(.movieRecommendations(id: id, page: page)) { result in
      switch result {
      case let .success(response):
        do {
          ...
          completion(
            RecommendedMovieResponse(results: convertedListItems,
                                     page: results.page,
